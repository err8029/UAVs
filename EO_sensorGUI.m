...
if (isempty(hfig))
    ...
    hmenu = uimenu('Label','&Comms for Radar','Tag','M');
		uimenu(hmenu,'Label','Calculate','Callback',@commsSAR_GUI,'tag','comms');   
...
function commsSAR_GUI(hco,eventStruct)
    
    hfig = figure;
	set(hfig,'numbertitle','off');               % treu el numero de figura
	set(hfig,'name',['Comms for radar']);
	set(hfig,'MenuBar','none');                  % treiem els menus i els icons
	set(hfig,'doublebuffer','on');               % dos buffers de grafics
	set(hfig,'CloseRequestFcn',@tancar3)          % funci� que es cridar� al tancar la finestra
	set(hfig,'tag','MOSTRARDADES');              % identifiquem la figura
    set(hfig,'Units','Normalized','Position',[0.3,0.3,0.3,0.4]);
    ...
    set(findobj('Tag','SARbw'),'String',['Bandwith needed for SAR: ' num2str(SARbw) ' Mbps']);
    ...
    set(findobj('Tag','GMTIbw'),'String',['Bandwith needed for GMTI: ' num2str(GMTIbw) ' Mbps']);

    %% Total bandwidth

    bw = SARbw + SAPbw + GMTIbw;
    set(findobj('Tag','bw'),'String',['Total Bandwith needed: ' num2str(bw) ' Mbps']);
    
    end
    
    function tancar3(hco,eventStruct)
        closereq;
    end
